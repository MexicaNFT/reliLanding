...
export default function Register() {
  ...
  useEffect(() => {
    const checkSubscription = async () => {
      ...
      try {
        const response = await fetch("/register/api/checkSubscription", {
          method: "POST",
          headers: {
        ...
        if (data.subscribed) {
          const portalResponse = await fetch(
            "/register/api/createCustomerPortalSession",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
    ...
    const fetchPlans = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/register/api/getPrices?interval=${interval}`
        );
        if (!response.ok) {
  ...
  const getUserEmail = async () => {
    try {
      ...
      setErrorMessage("Error retrieving user email.");
    }
  };
